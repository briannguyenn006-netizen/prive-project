...
def build_pro_article(title, summary):
    ...
    # Giữ nguyên độ dài 1000 chữ sếp ưng
    prompt = f"""
...
def run():
    ...
    # TRADINGVIEW (Giữ nguyên cái dải chạy xịn xò)
    tv_ticker = """
    ...
    for i, e in enumerate(feed.entries[:12]):
        ...
        # TRANG BÀI VIẾT: CHỈ SỬA LOGO SIZE
        with open(path, 'w', encoding='utf-8') as f:
            ...
            <style>
                ...
                /* FIX ĐÚNG CHỖ NÀY: Logo nhỏ lại cho nghệ */
                .news-logo{{ width: 40px; height: 40px; object-fit: contain; }}
    ...
    # TRANG CHỦ: GIỮ NGUYÊN LAYOUT NGHỆ THUẬT
    with open('index.html', 'w', encoding='utf-8') as f:
        ...
        {tv_ticker}
        <header><a href='#' class='logo'><img src='{FAVICON_PATH}'>BROKENOMORE</a><div style='text-align:right;color:#fbbf24;font-weight:900'>LIVE TERMINAL // 03:00 AM</div></header>
        <div class='main'>
            ...
            <div style='border-left:1px solid #222;padding-left:40px'><h2 style='color:#fbbf24;font-size:12px;text-transform:uppercase;border-bottom:1px solid #333;padding-bottom:15px;letter-spacing:2px'>Intelligence Grid</h2>{grid_html}</div>
            <div style='border-left:1px solid #222;padding-left:40px'><h2 style='color:#fbbf24;font-size:12px;text-transform:uppercase;border-bottom:1px solid #333;padding-bottom:15px;letter-spacing:2px'>Global Feed</h2><ul>{"".join([f"<li style='margin-bottom:20px;border-bottom:1px solid #111;padding-bottom:10px'><a href='./{a['p']}' style='color:#999;text-decoration:none;font-size:15px;font-weight:bold'>{a['t']}</a></li>" for a in articles[5:]])}</ul></div>
        </div></body></html>""")
